...
{
    [InitializeOnLoad] public static class BVisualFixValidation
    {
        const string Key="CoinMerge.BVisualFix.Pages",Prefix="coinmerge.bvisualfix.pages.disposable";
        ...
        static string Output=>Path.GetFullPath("Design/BVisualFix20260917/After");
        ...
        static BVisualFixValidation(){EditorApplication.playModeStateChanged+=State;EditorApplication.update+=Poll;}
        static void Poll(){const string request="Temp/BVisualFixValidation.request";if(!File.Exists(request)||SessionState.GetBool(Key,false)||EditorApplication.isCompiling||EditorApplication.isUpdating)return;if(EditorApplication.isPlaying){EditorApplication.ExitPlaymode();return;}if(EditorApplication.isPlayingOrWillChangePlaymode)return;SessionState.SetBool(Key+".all",File.ReadAllText(request).Contains("all-locales"));File.Delete(request);Run();}
        [MenuItem("Coin Merge/Reskin/Validate B visual fixes and localization")]
        public static void Run()
        {
            Directory.CreateDirectory(Output);SessionState.SetBool(Key,true);SessionState.SetString(Key+".error","");
            var store=new PlayerStore(Prefix);store.ResetPlayer();store.ResetProfile();store.Save(new PlayerProgress{guideStep=5,fakeMoney=488.49,coin1024Number=50,gameTotalScore=28});store.SaveProfile(new VersionProfile{country="US",cohort="B",rewardedVariant=true});
        ...
        static void Tick()
        {
            ...
            try
            {
                ...
                if(!s){s=UnityEngine.Object.FindObjectOfType<RecoveredGameSession>();camera=s.worldCamera;Resize(1080,2340);var list=new List<string>{"US"};if(SessionState.GetBool(Key+".all",false)){list.Add("RU");list.Add("JP");foreach(var c in s.board.Config.rules.supportedCountries)if(!list.Contains(c))list.Add(c);}countries=list.ToArray();}
                ...
                if(localeIndex<3||countries[localeIndex]=="BR"&&step==7||countries[localeIndex]=="ID"&&step==8)Capture(countries[localeIndex]+"_"+Name(step)+".png");
                if(step==2){s.menus.ruleAnimation.EvaluateAt("animation",3.3333f);Render();Capture(countries[localeIndex]+"_rules_source_end.png");}
                if(step==17 && s.wheelView.countLabel.gameObject.activeInHierarchy)throw new Exception("Decorative wheel count was visible");
                if(step==4)
                {
        ...
        {
            if(index==0)return;if(index==3){s.menus.Act(7);return;}if(index==2){s.menus.Act(2);return;}if(index<=11){s.menus.Show(index-1);return;}if(index<=16){s.rewardView.Show(index-11,6.72);return;}
        ...
        {
            Hide();s.ChangeProfile("US","B",true);s.rating.Show();Click(s.rating.stars[3]);if(s.rating.SelectedIndex!=3)throw new Exception("Rating state not updated");Render();Capture("US_rating_selected.png");Click(s.rating.close);
            foreach(bool terms in new[]{false,true})
            {
                s.menus.Act(terms?8:7);var scroll=(terms?s.menus.termsScroll:s.menus.privacyScroll).GetComponent<ScrollRect>();
                if(scroll.content.name!="NativePolicyText")throw new Exception("Policy still uses a bitmap");
                foreach(var t in scroll.content.GetComponentsInChildren<Text>())if(t.text.Contains("Coin Merge Fortune"))throw new Exception("Old product name in policy");
                foreach(float position in new[]{1f,.5f,0f}){scroll.verticalNormalizedPosition=position;Render();Audit("Native policy scroll "+position);Capture((terms?"Terms":"Privacy")+"_"+position.ToString("F1",System.Globalization.CultureInfo.InvariantCulture)+".png");}
                if(scroll.content.rect.height<=scroll.viewport.rect.height)throw new Exception("Policy cannot scroll through complete content");s.menus.CloseAll();
            }
            checks.Add("Both policies: native paragraphs, current product name, top/middle/bottom scroll verified");
        ...
        static void Supplement()
        {
            ...
            if(extra==2)
            {
                if(s.menus.CurrentPage==8){VerificationBounds();if(EditorApplication.timeSinceStartup>deadline)throw new Exception("Verification animation timed out");Render();Capture("US_verification_flow.png");next=EditorApplication.timeSinceStartup+.3;return;}
            ...
            if(extra==3)
            {
                ...
                checks.Add("B-only verification complete; A scene was not opened");Finish(null);return;
            }
            if(extra>=100)
            {
                ...
                int page=extra-100;if(page==0){while(packaged.Dialog>=0)packaged.Act(0);packaged.Home();Render();Capture("A_home.png");packaged.ChooseMode(0);Render();Capture("A_gameplay.png");}if(page>=packaged.dialogs.Length){checks.Add("A base version: all 7 native dialogs rendered and text bounds audited");Finish(null);return;}
        ...
        static void VerificationBounds()
        {
            Render();var tips=s.menus.verification.tips;
            if(tips.Length<4||tips[2].localScale.x<.95f||tips[3].localScale.x<.95f)return;
            var a=tips[2].GetComponent<Text>();var b=tips[3].GetComponent<Text>();
            var ar=ScreenBounds(a);var br=ScreenBounds(b);
            if(ar.Overlaps(br))throw new Exception("Verification title overlaps explanation: "+ar+" / "+br);
            if(!checks.Contains("Animated verification title and explanation remain separate"))checks.Add("Animated verification title and explanation remain separate");
        }
        static Rect ScreenBounds(Text t)
        {
            var mesh=t.canvasRenderer.GetMesh();var b=mesh.bounds;
            var a=RectTransformUtility.WorldToScreenPoint(camera,t.transform.TransformPoint(b.min));
            var z=RectTransformUtility.WorldToScreenPoint(camera,t.transform.TransformPoint(b.max));
            return Rect.MinMaxRect(a.x,a.y,z.x,z.y);
        }
        ...
        static void Capture(string filename){Inventory(filename);
var old=RenderTexture.active;RenderTexture.active=target;var t=new Texture2D(target.width,target.height,TextureFormat.RGB24,false);t.ReadPixels(new Rect(0,0,target.width,target.height),0,0);t.Apply();File.WriteAllBytes(Path.Combine(Output,filename),t.EncodeToPNG());RenderTexture.active=old;UnityEngine.Object.DestroyImmediate(t);}
        
        [Serializable] sealed class CaptureInventory{public string screen;public List<VisualItem> items=new List<VisualItem>();}
        [Serializable] sealed class VisualItem{public string kind,path,asset,text,color;public float width,height,fontSize;public bool preserveAspect;}
        static void Inventory(string filename)
        {
            var root=packaged?packaged.gameObject:s.gameObject;var report=new CaptureInventory{screen=filename};
            foreach(var im in root.GetComponentsInChildren<Image>())if(im.enabled&&im.sprite&&!im.canvasRenderer.cull)report.items.Add(new VisualItem{kind="Image",path=PathOf(im.transform),asset=AssetDatabase.GetAssetPath(im.sprite),color=ColorUtility.ToHtmlStringRGBA(im.color),width=im.rectTransform.rect.width,height=im.rectTransform.rect.height,preserveAspect=im.preserveAspect});
            foreach(var tx in root.GetComponentsInChildren<Text>())if(tx.enabled&&tx.font&&!tx.canvasRenderer.cull)report.items.Add(new VisualItem{kind="Text",path=PathOf(tx.transform),asset=AssetDatabase.GetAssetPath(tx.font),text=tx.text,color=ColorUtility.ToHtmlStringRGBA(tx.color),width=tx.rectTransform.rect.width,height=tx.rectTransform.rect.height,fontSize=tx.fontSize});
            File.WriteAllText(Path.Combine(Output,Path.GetFileNameWithoutExtension(filename)+".json"),JsonUtility.ToJson(report,true));
        }
