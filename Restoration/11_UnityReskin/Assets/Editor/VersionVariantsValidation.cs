            ...
            try
            {
                ...
                switch(phase++)
                {
                    case 0:
                        ...
                        hot.Player.fakeMoney=77;hot.Save();Click(gm.open);Click(gm.baseVersion);gm.Draft.country="JP";
                        Require(gm.Draft.contentMode==VersionRouting.Packaged&&!gm.Draft.rewardedVariant,"Visible A button selects real base content independently of original cohort");
                        Require(gm.cohortValue.GetComponent<Button>()==null,"Original cohort is a read-only label, not a misleading version switch");
                        ...
                        Click(gm.open);Click(gm.baseVersion);gm.Draft.country="JP";Click(gm.apply);next=EditorApplication.timeSinceStartup+2;break;
                    case 1:
                        BindScene();Require(basic&&SceneManager.GetActiveScene().name=="RecoveredPackaged","Apply loads actual packaged scene");
                        Require(router.Profile.country=="JP"&&router.Profile.cohort=="B"&&!router.Profile.rewardedVariant,"Country, cohort and content persist across scenes");
                    ...
                    case 4:
                        ...
                        Click(gm.open);Click(gm.reset);Require(basic.Player.heart==10&&basic.Player.coins==0&&router.Profile.country=="JP"&&router.Profile.cohort=="B"&&!router.Profile.rewardedVariant,"Reset clears current player while preserving country/cohort/content");
                        Click(gm.rewardedVersion); // Explicit local B selects the rewards scene.
                        Require(gm.Draft.contentMode==VersionRouting.Rewarded&&gm.Draft.rewardedVariant,"Visible B button selects real rewards content");
                    ...
                    case 5:
                        ...
                        Require(hot.Profile.country=="JP"&&hot.Profile.cohort=="B","Rewarded runtime reads applied country and cohort");
