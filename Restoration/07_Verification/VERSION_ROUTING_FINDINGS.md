...
- `VersionGM.prefab`：650 × 890 的居中卡片，参考画布 750 × 1624，中文界面。仅遮罩覆盖全屏，内容卡片不全屏；右下侧 GM 按钮可打开，X/遮罩可取消，应用才保存并重进场景。
- 内容版本（基础 / 收益 / 自动）、原始 AB 标签（A / B / 自动）、26 国地区分别设置。
...
- `version_variants_validation.json`：77 项实际 Unity Play Mode 检查，包括 EventSystem 指针命中、版本场景加载、独立存档、清档持久化、地区/比例按钮、基础版多边形物理合成、解锁、体力购买、失败重试和音乐开关。
