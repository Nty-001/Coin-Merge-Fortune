...
交付入口：[离线 HTML 测试指南](../09_TestGuide/index.html)。包含奖励时间轴、广告回调矩阵、按国家/档位切换的提现任务图、活跃日模拟、GM 截图和 42 项可记录结果的人工用例。无需服务器或安装依赖。
...
- [html_guide_validation.json](html_guide_validation.json)：15 项离线源代码检查通过，含完整脚本语法、内嵌报告新鲜度、42 个用例、72 组地区组/档位/路线配置和活跃日模拟逻辑。
...
首奖评分、复活标题和1_A未启动时计数已在后续生命周期核对中修复；最高金币改为原飞入入口流程，未新增奖励弹窗。SDK/远程任务墙缺失响应仍保留证据边界。商业 SDK 保持 Mock，不联网执行广告投放或付款。
