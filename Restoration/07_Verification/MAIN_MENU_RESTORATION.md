...
## 已打开编辑器中的旧场景复验（2026-09-15）

本次在用户实际打开的 `06_UnityFramework` 编辑器窗口复现了“重新 Play 后设置仍然无响应”。磁盘场景已更新，但编辑器继续从内存里的旧场景进入 Play；仅停止再运行不会重新读取磁盘场景。旧实例的日志还反复出现 `VersionGmPanel.Awake` / `OnDestroy` 空引用异常。此前验证工程通过，并不能证明这个已打开的编辑器已经使用新版场景。

处理时先将旧场景快照备份到本地忽略文件，再停止 Play，通过 Unity 的 Load Scene 重新打开正式工程 `Assets/Scenes/RecoveredMain.unity`。没有重置玩家进度或修改提现门槛，也没有添加 Editor 专用运行时兜底。

在同一个正式工程窗口，使用实际鼠标点击完成以下复验：

| 操作 | 实际观察 |
| --- | --- |
| 设置 → 隐私说明 → 关闭 → 关闭设置 | 两层弹窗依次打开和返回 |
| 规则 → OK | 规则弹窗打开，合成顺序动画显示完整，确认返回 |
| 顶部 Withdraw → Withdraw → 返回 | 现金页面打开，未达门槛显示剩余金额提示，返回有效 |
| 2000 金币 → 切换金额档位 → 返回 | 金币页面打开，选择状态和对应条件随档位更新，返回有效 |
| GM → 关闭 | 新版 A/B 按钮、国家与当前版本正常显示；关闭有效 |
| 停止 Play → 再次 Play → 设置 | 设置弹窗仍能打开，已有玩家进度保留 |

从重新载入后的首次 Play 开始，到上述第二次 Play 复验结束，新增 Editor 日志未出现异常。已有物理碰撞体 density 警告仍存在，本次没有把它当成菜单故障，也没有声称该警告已修复。

当前用户存档未达到首档金币提现条件，所以本次没有强行进入账户验证或改写玩家数据。达到条件后的账户输入、验证完成与阶段存档，仍以既有 `recovered_menus_validation.json` 的独立测试结果为依据；不能把这些结果写成当前用户存档已手动走通账户验证。

后续在外部生成或同步 Scene / Prefab 时，交付前必须检查用户实际打开的编辑器：保存或备份未提交编辑，停止 Play，显式重新载入目标场景，并真实点击入口，再停止／Play 复验。不能仅凭磁盘文件一致或独立验证工程通过，就报告当前窗口已经可用。Unity 的临时场景快照可能采用二进制序列化，不能通过搜索 YAML 字段或 GUID 文本缺失来判断其中组件不存在。
