...
namespace CoinMerge.Recovery
{
    public sealed class GameplayGmPanel : MonoBehaviour
    {
        ...
        CashGroup Group=>RecoveredGameRules.CashConfiguration(owner.router.balance.rules,owner.router.Profile.country);
        WithdrawProduct Product=>Group.new_Fake_products[selectedProduct];
        static readonly string[] Tasks={"最高金币","广告次数","现金余额","有效活跃天数","视频次数"};
        static readonly string[] Outcomes={"成功","取消","无填充","失败"};
        void Awake()
        {
        ...
        public void Execute(int code)
        {
            ...
            switch(code)
            {
                case 10:selectedProduct=(selectedProduct+Group.new_Fake_products.Length-1)%Group.new_Fake_products.Length;break;
                case 11:selectedProduct=(selectedProduct+1)%Group.new_Fake_products.Length;break;
                case 12:selectedTask=(selectedTask+4)%5;break;
                case 13:selectedTask=(selectedTask+1)%5;break;
                ...
                case 24:owner.Hide();Session.menus.CloseAll();Session.menus.Act(3);break;
        ...
        public void Refresh()
        {
            ...
            taskLabel.text="任务 "+(selectedTask+1)+" / "+Tasks[selectedTask];
            progressStatus.text=PlayerClock.Now(Player).ToString("yyyy-MM-dd HH:mm")+"  偏移 +"+(Player.gmTimeOffsetSeconds/3600).ToString("0.#")+"h\n"+
                "现金 "+Session.Locale.Money(Player.fakeMoney)+" | 最高币 "+Player.coin1024Number+" | 广告 "+Player.watch_video_count+"\n"+
                "有效天 "+Player.loginDays+" | 今日最高币 "+Player.today1024NumberCoin+"/"+owner.router.balance.rules.flow.validLoginMergeCount+"\n"+
                "实际任务 "+Math.Min(6,Player.newFakeMoneyWithdraw[selectedProduct]+1)+"（6=全部达标）\n"+message;
            ...
            eventStatus.text="窗口投币 "+Player.windowsCointimes+" | 总投币 "+Player.dropCointimes+" | 转盘次数 "+Player.currentLotteryCount+"\n"+
                "Mock："+Outcomes[(int)Session.Sdk.NextAdOutcome]+" | "+last+"\n"+message;
            var reward=owner.router.balance.rules.lotteryRewards[selectedSlot];slotLabel.text="下次格子 "+(selectedSlot+1)+" / "+reward.type+" / 权重 "+reward.probability;
        }
