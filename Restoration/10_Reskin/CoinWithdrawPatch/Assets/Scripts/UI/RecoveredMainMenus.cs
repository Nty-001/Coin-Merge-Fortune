...
namespace CoinMerge.Recovery
{
    ...
    [DefaultExecutionOrder(100)] public sealed class RecoveredMainMenus : MonoBehaviour
    {
        ...
        void ShowPolicy(bool terms){Show(Policy);privacyScroll.SetActive(!terms);termsScroll.SetActive(terms);policyTitle.text=Locale.Label(terms?"6":"7");}
        void RefreshFake()
        {
        ...
        void RefreshCoin()
        {
            ...
            Fill(coinFill,progress);if(coinPercent)coinPercent.text=Player.newFakeMoneyWithdraw[SelectedCoin]==0?Math.Min(Player.coin1024Number,products[SelectedCoin].condition_merge)+"/"+products[SelectedCoin].condition_merge:Percent(progress);
