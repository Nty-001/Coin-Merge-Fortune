...
for p in sorted((WORK/'Assets').rglob('*')):
 ...
 if rel.as_posix().startswith('Assets/Editor/Reskin'):continue
 original=BASE/rel
...
tests={}
for name,file in [('menus','recovered_menus_validation.json'),('gameplay','native_gameplay_validation.json'),('reskin','ReskinRuntime/checks.json')]:
...
html+='<p>编辑工程：UnityWorkingCopy；场景：Assets/Scenes/RecoveredMain.unity。点击截图查看原尺寸。</p></html>'
